        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.notifications.data.length > 0) {
                    ...
                    setTimeout(function () {
                        $('#notification-button').attr('title', `Hi, Kamu punya ${response.notifications.total} message`).addClass('animated swing text-danger').tooltip('show');
                        hideTooltips('#notification-button')
                    }, 1000);

                } else {
                    head_notify.html(`<li class="text-center"><i class="fa fa-smile-o fa-2x text-primary"></i><a href="#" class="mt-0">No unread notifications</a></li>`);
                    $('#notification-button').attr('title', `Hi, Kamu punya ${response.notifications.total} message`).removeClass('animated swing text-danger');
                }
            },
            error: function () {
                $('.notification-list').html("<p class='text-center'>Error load notification</p>");
