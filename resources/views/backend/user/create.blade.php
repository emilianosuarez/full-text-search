    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-6 form-group">
                ...
                {!! html()->text('first_name')->placeholder('Type here...')->class('form-control')->id('first_name')->required() !!}
            </div>
            <div class="col-md-6 form-group">
                {!! html()->label('Last Name','last_name')->class('control-label') !!}
                <span class="text-dark">(optional)</span>
                {!! html()->text('last_name')->placeholder('Type here...')->class('form-control')->id('last_name') !!}
            </div>
        </div>
        <div class="form-group">
            {!! html()->label('Email','email')->class('control-label') !!}
            <span class="text-danger">*</span>
            {!! html()->text('email')->placeholder('Type here...')->class('form-control')->id('email')->required() !!}
        </div>
        <div class="form-group row">
            <div class="col-md-6">
                {!! html()->label('Password','password')->class('control-label') !!}
                <span class="text-danger">*</span>
                {!! html()->password('password')->placeholder('Type password')->class('form-control')->id('password')->required() !!}
            </div>
            <div class="col-md-6">
                {!! html()->label('Password Confirmation','password_confirmation')->class('control-label') !!}
                <span class="text-danger">*</span>
                {!! html()->password('password_confirmation')->placeholder('Retype password')->class('form-control')->id('password_confirmation')->required() !!}
            </div>
        </div>
        <div class="form-group row">
            <div class="col-md-6">
                {!! html()->label('Level Id','level_id')->class('control-label') !!}
                <span class="text-danger">*</span>
                {!! html()->select('level_id',$level)->class('form-control select2')->id('level_id')->required() !!}
            </div>
            <div class="col-md-6">
                {!! html()->label('Access Group Id','access_group_id')->class('control-label') !!}
                <span class="text-danger">*</span>
